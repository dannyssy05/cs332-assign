...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    for ((nextBlock, move) <- b.legalNeighbors.toStream)
    yield (nextBlock, move :: history)
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case _ =>
      val next = for {
        ...
        (nextBlock, newHistory) <- neighborsWithHistory(block, history)
        if !explored(nextBlock)
      } yield (nextBlock, newHistory)
