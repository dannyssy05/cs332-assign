...
trait NodeScala {
   ...
   */
  def start(relativePath: String)(handler: Request => Response): Subscription ={val listener = createListener(relativePath)
    val listenerSubscription = listener.start()


    val worker = Future.run() { CancellationToken =>
      Future {
        while (CancellationToken.nonCancelled) {
          val next= listener.nextRequest()
          next.foreach { case (request, exchange) =>
    ...
    Subscription(listenerSubscription, worker)}
